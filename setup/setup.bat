...
:: ── 2. Smoke test ──────────────────────────────────────────
if %PYTHON_OK% == 0 goto skip_smoke
...
if %FAIL% == 0 (
    ...
    :: Write VS Code settings if VS Code is present (single line — no ^ continuation)
    code --version >nul 2>&1
    if %errorlevel% == 0 (
        if not exist ".vscode" mkdir .vscode
        powershell -NoProfile -Command "$s=[ordered]@{'python.defaultInterpreterPath'='python-runtime/windows/python.exe';'terminal.integrated.env.windows'=@{'PATH'='${workspaceFolder}/python-runtime/windows;${env:PATH}'}};$s|ConvertTo-Json -Depth 3|Set-Content '.vscode/settings.json' -Encoding UTF8" >nul 2>&1
    )
...
if %ERR_PYTHON% == 1 (
    echo   ISSUE: Python runtime is missing.
    echo   FIX:   Make sure the full workshop folder was unzipped, or
    echo          check your internet connection and re-run this script.
    echo.
)

if %ERR_SMOKE% == 1 (
    echo   ISSUE: The music data file failed to load.
    echo   FIX:   Make sure the full workshop folder was unzipped before
